<?php
	...
	if(isLoggedIn())
	{
		require("db_connection.php");
					$map_statement = "SELECT positioncoordinates FROM registerdevice WHERE domainid1 = '$userdomain'";
					$map_query = mysqli_query($con, $map_statement);	
					$map_array = array();
					while ($map_row = mysqli_fetch_array($map_query))
					{
						$map_array[] = $map_row['positioncoordinates'];
	...
	{
		die ("Error: You are not authorized to view this page.");
	}
          ...
          <head>
            ...
            <script>
			  ...
			  function initialize() {
				...
				var positioncoordinates = [<?php echo "$map_string" ?>];
				var i=0, length=positioncoordinates.length;
				while(i<length){
					var marker = new google.maps.Marker({
						position: new google.maps.LatLng(positioncoordinates[i], positioncoordinates[i+1]),
						map: map
					});
					i++;
					i++;
			  ...
			  function realTime() {
				 $.ajax({
					  url: "processcoordinates.php",
					  dataType:"json",
					  async:false,
					  cache:false,
					  success: function(dataresponse)
					  {
						  var obj = dataresponse;
						  //var stringObj = JSON.stringify(obj);
						  //var removequotes = stringObj.replace(/\"/g,"");
						  var location = dataresponse.split(',');
			  ...
			  function resetMarkers(markersArray){
